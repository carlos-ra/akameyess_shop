...
export const CartButton = ({ product, className = '' }) => {
    ...
    if (cartItem) {
        return (_jsxs("div", { className: "quantity-controls", children: [_jsx("button", { onClick: () => handleUpdateQuantity(cartItem.quantity - 1), className: "quantity-btn", children: "-" }), _jsx("span", { className: "quantity", children: cartItem.quantity }), _jsx("button", { onClick: () => handleUpdateQuantity(cartItem.quantity + 1), className: "quantity-btn", children: "+" })] }));
    }
    return (_jsx("button", { onClick: handleAddToCart, className: `cart-button ${className}`, children: "Add to Cart" }));
};
